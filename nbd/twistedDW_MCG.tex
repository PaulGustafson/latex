...
%%%%%%%%%%%%%%%%%%%%%%
\begin{defn}\label{d:coloring} Let $\Si$ an oriented surface
...
\begin{defn}
The representation space associated to a closed, oriented surface $M$ by a spherical category $\mathcal A$ is the vector space of formal linear combinations of $\mathcal A$-colored graph embeddings modulo the subspace spanned by the null graphs.
\end{defn}
...
\begin{proof}
Let $\Gamma$ be a $\Vect_G^\omega$-colored graph embedding, and let $g \ge 1$ be the genus of $M$ (if $g = 0$, the mapping class group is trivial). Thinking of $M$ as a quotient of its fundamental $4g$-gon, by isotopy we may assume vertices of $\Gamma$ lie in the interior of the polygon and that all the edges of $\Gamma$ do not intersect corners and meet the sides transversally.  Evaluating on the interior of the polygon shows that $\Gamma$ is equivalent to a graph with a single vertex whose edges are simple closed curves, each of which intersect the boundary of the polygon precisely once.  By using the local relations, we can replace all the edges intersecting a side with a single edge labeled by the tensor product of their labels.  If there are no edges intersecting a side, we can insert a single edge labeled by the group identity into $\Gamma$ that intersects only that side.  Thus, $\Gamma$ is equivalent to a colored graph with one vertex $v$ and edges $e_1, \ldots, e_{2g}$ corresponding to the standard generators of $\pi_q(M,v)$ as shown in Figure \ref{fig:span}.
...
\caption{First type of Dehn twist. The figures are ordered from left to right, then top to bottom.}
...
\caption{Second type of Dehn twist. The figures are ordered from left to right, then top to bottom.}
...
When $M$ has boundary, we define the mapping class group $\MCG(M)$ to be the group of isotopy classes of homeomorphisms fixing the boundary of $M$ setwise. There are mapping class group representations for every labelling of the boundary components of $M$ by objects in $Z(\vgo) \simeq \Mod(D^\omega(G))$.  Given such a labelling, the representation space is spanned by colored graphs for which the coloring of a leg ending in a boundary component coincides with the image of the forgetful functor $F: Z(\vgo) \to \vgo$ on the labelling of that boundary component.  The same local relations are valid in this representation space, along with an additional local relation around each boundary component that we will not need \cite{kirillovStringNets}.
...
Let $N$ denote the closed surface obtained by filling in all the boundary components of $M$ with disks. The mapping class group $\MCG(M)$ is generated by the same Dehn twists as for $\MCG(N)$, as well as braids interchanging boundary components and mapping classes corresponding to dragging a boundary component along a representative of a standard generator of $\pi_1(N)$ \cite{birman}.  It is straightforward to check that applying any of these generators of $\MCG(M)$ to a colored graph in $S$ yields an element in $\Img(\omega)S$.  Since the braid group is also generated by such braids, we have the following theorem.
...
\caption{A braid generator. The figures are ordered from left to right, then top to bottom.}
\label{fig:braid1}
\end{table}
...
\begin{tabular}{|c|c|}
  ...
  % Drag 1
    ...
    mark=at position 0.5 with {\arrow{>}}}
      ...
      % cut for Dehn twist
      \draw[loosely dashed] \lcut -- \boundaryComponent --\rcut;
      

      %graph edges
      \draw[postaction={decorate}]   \mv -- \boundaryComponent node[pos=.5,sloped,below]{$k$};
      \draw[postaction={decorate}]   \rv -- \revertex node[pos=.5,sloped,below]{$h$};
      \draw[postaction={decorate}]   \rv -- \lv node[pos=.5,below]{$p$};
      \draw[postaction={decorate}]   \uv -- \rv  node[pos=.5,sloped,above]{$ph$};
      \draw[postaction={decorate}]   \mv -- \uv node[pos=.5,left]{$phg$};
      \draw[postaction={decorate}]   \uv -- \mtop node[pos=.5,left]{$g$};
      \draw[postaction={decorate}]   \lev -- \mv node[pos=.5,sloped,below]{$h$};
      \draw[postaction={decorate}]   \lv -- \mv node[pos=.5,left]{$q$};
      \draw[postaction={decorate}]  (0,\dby)  -- \lv node[pos=.5,left]{$p^{-1}q$};
...
&
  ...
  % Drag 2
    ...
    mark=at position 0.5 with {\arrow{>}}}
      ...
      %graph edges
      \draw[postaction={decorate}]   \mv -- \reTwov node[pos=.5,sloped,right,red]{$k$};
      \draw[postaction={decorate}]   \leTwov -- \boundaryComponent node[pos=.5,sloped,left,red]{$k$};

      \draw[postaction={decorate}]   \rv -- \revertex node[pos=.5,sloped,below,red]{$h$};
      \draw[postaction={decorate}]   \rv -- \lv node[pos=.5,below,red]{$p$};

      \draw[postaction={decorate}]   \reOnev -- \rv   node[pos=.5,sloped,right,red]{$ph$};
      \draw[postaction={decorate}]    \phInt -- \leOnev  node[pos=.5,sloped,left,red]{$ph$};
      \draw[postaction={decorate}]     \leFivev -- \phInt  node[pos=.5,sloped,above,red]{$ph$};
      \draw[postaction={decorate}]    \uv -- \reFivev    node[pos=.5,sloped,above,red]{$ph$};

      \draw[postaction={decorate}]   \mv -- \reThreev node[pos=.5,left,red]{$phg$};
      \draw[postaction={decorate}]   \leThreev -- \phgInt node[pos=.5,left,red]{$phg$};
      \draw[postaction={decorate}]   \phgInt -- \leFourv node[pos=.5,left,red]{$phg$};
      \draw[postaction={decorate}]   \reFourv -- \uv node[pos=.5,sloped,right,red]{$phg$};

      \draw[postaction={decorate}]   \uv -- \mtop node[pos=.5,left,red]{$g$};
      \draw[postaction={decorate}]   \lev -- \mv node[pos=.5,sloped,below,red]{$h$};
      \draw[postaction={decorate}]   \lv -- \mv node[pos=.5,left,red]{$q$};
      \draw[postaction={decorate}]  (0,\dby)  -- \lv node[pos=.5,left,red]{$p^{-1}q$};
...
\caption{Pulling a boundary component along a generator for the fundamental group of the corresponding closed surface. The figures are ordered from left to right, then top to bottom.}
\label{fig:drag1}
\end{table}


\begin{thm}\label{thm:compact}
The image of any twisted Dijkgraaf-Witten representation of a mapping class group of an orientable, compact surface $M$ is finite.  In addition, any such braid group representation is also finite.
\end{thm}
...
The next step towards the Property F conjecture is to consider more complicated spherical categories than $\vgo$.  The Tambara-Yamagami categories \cite{tambara} should be a good candidate.  The main additional complication here is the appearance of multifusion channels, i.e. the tensor product of two simple objects can be a direct sum of multiple simple objects.  
\medskip
