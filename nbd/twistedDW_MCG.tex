...
\nc{\makeBase}{
      % left loop
       ...
       \draw  \mv -- ({\leftX - 4*\loopR}, \pY);
       \draw   ({\leftX - 2*\loopR}, \pY) -- \mv;
 
       % right leg
       \draw  \mv -- ({\leftX - 3*\loopR}, \pY);       
...
\nc{\makeBraid}{
     ...
     \begin{scope}[very thick,decoration={
       ...
       % right loop
       \draw[postaction={decorate}]     plot[domain=180:0] ({2*\loopR*cos(\x) + \leftX}, {2*\loopR*sin(\x) + \pY});
       \node[above] at  ({2*\loopR*cos(90) + \leftX}, {2*\loopR*sin(90) + \pY}) {$g$};
       \draw[postaction={decorate}]      plot[domain=0:180] ({4*\loopR*cos(\x) + \leftX}, {4*\loopR*sin(\x) + \pY});
       \node[above] at  ({4*\loopR*cos(90) + \leftX}, {4*\loopR*sin(90) + \pY}) {$g$};
       \draw[postaction={decorate}]      plot[domain=180:360] ({\loopR*cos(\x) + \leftX + 3*\loopR}, {\loopR*sin(\x) + \pY});
