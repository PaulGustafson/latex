...
In Figure \ref{fig:tikzTwist1_3}, we add the upper left vertex, which is labeled by a $\coev_g$.  We then connect the vertices with an unlabelled edge, which is shorthand for labelling by the object $1$ .  At this point, the vertices are in states $hg1h^{-1}(hgh^{-1})$ and $gg^{-1}1$.  To compose the two vertices, we must use the spherical structure on the former vertex and reassociate until it is in the state $h^{-1}(hgh^{-1})hg1$.  In doing so we pick up a factor of 
