...
following data:
  ...
  \begin{itemize}
    ...
    \item Choice of a vector $\ph(v)\in \Hom (1, V(\ee_1) \otimes \dots \otimes V(\ee_n))$  for   every vertex $v$, where 
...
Given the fact that the braid group representations associated to $D^\omega(G)$ are also finite \cite{erw}, it should be straightforward 
 to prove that mapping class group representations associated to $\Vect_G^\omega$ for any orientable surface, including boundary, are finite.  
...
One could also consider more complicated spherical categories than $\Vect^G_\omega$, such as the 
