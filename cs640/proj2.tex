...
%
% Probably the best idea is to try to reduce from arbitrary subgroups $H$
% to normal subgroups, such as $G'H$ in the Ivanyos paper.  We can try to 
% do this in the 4x4-unipotent, upper-triangular case.
%
%
% If we want to use an extension of the finite Heisenberg group $H := \Heis(\ZZ_p)$
% as a toy example for the nil-3 case, we should look at automorphisms
% $\phi: G \to \Aut(H)$, for some group $G$.  
% The arguments of the first paper above should imply
% that $\Aut(H)$ is a semidirect product  $(\ZZ_p \oplus \ZZ_p) \rtimes
% GL_2(\ZZ_p)$.  They give a fairly explicit description of the action.
% The inner automorphism group $\Inn(H)$ is the normal subgroup $\ZZ_p \oplus \ZZ_p$.
% 
% The simplest thing to do is to calculate the lower central series for
% the semidirect product $H \rtimes \Z_p$, where the $\Z_p$-action on $H$
% is the same as the action of a factor of $\Inn(H)$.  This doesn't seem like it
% does much, so I suspect that this new semidirect product group will be nil-2.
% If that doesn't work, we can start looking at maps into $GL_2(\ZZ_p)$.
%
% If we're feeling more ambitious, we can try to look at the definition of
% of the free nil-3 group on $n$ generators in the link to Terry Tao's blog,
% and try to construct "nice" automorphisms that generalize the ones in the 
% Ivanyos paper.
%
%
% Useful links:
%
% Survey paper:
% http://journals.aps.org/rmp/pdf/10.1103/RevModPhys.82.1
%
% Automorphisms of the discrete Heisenberg group: 
% https://arxiv.org/pdf/1505.00348.pdf
% 
% Terry Tao blog post on free nilpotent groups: 
% https://terrytao.wordpress.com/2009/12/21/the-free-nilpotent-group/
%
...
\title{The Hidden Subgroup Problem in Special Classes of Nil-3 Groups}


% author names and affiliations
% use a multiple column layout for up to three different
% affiliations
\author{\IEEEauthorblockN{Paul Gustafson}
\IEEEauthorblockA{Department of Mathematics\\
Texas A\&M University\\
pgustafs@math.tamu.edu}
\and
\IEEEauthorblockN{Julia Plavnik}
\IEEEauthorblockA{Department of Mathematics\\
Texas A\&M University\\
julia@math.tamu.edu}
\and
\IEEEauthorblockN{Qing Zhang}
\IEEEauthorblockA{Department of Mathematics\\
Texas A\&M University\\
