...
%\newcommand{\eadr}{\email}

%% \usepackage{amsmath,amssymb,fullpage,mathptmx, hyperref,dsfont,framed, graphicx, textcomp} % The usual suspects
%\usepackage{hyperref}
...
\newtheorem{uconj}[thm]{Conjecture}
...
Much more is known for modular categories other than $\Mod(D^\omega(G))$, especially for braid group representations.  More precisely, the braid group Property F conjecture has been verified for all unitary modular tensor categories arising from the classical quantum group construction and various cases related to Hecke- and BMW-algebras \cite{FRW, FLW, J86, J89, LRW, r, rw}.

Less is known about the more general mapping class group conjecture.  Wright has shown that, for any closed surface, the mapping class group representations associated to $U_q(sl(2))$ at level $4$ have finite image \cite{gw}.  Funar proved that, for closed surfaces of genus $g \ge 2$, the mapping class group representations associated $SU(2)$ and $SO(3)$ are infinite for all sufficiently large levels \cite{funar}.



\section{Background}
\subsection{The spherical fusion category $\Vect_G^\omega$}
The following definitions are well-known and can be found in \cite{etingofTensor}.  Let $k$ be an  algebraically closed field of characteristic 0,  $G$ a finite group, and $\omega \in Z^3(G, k^\times)$ a 3-cocycle.    The spherical fusion category of $G$-graded $k$-vector spaces with associativity defined by $\omega$ is denoted $\Vect_G^\omega$.  The objects of this category are vector spaces with a decomposition $V = \bigoplus_{g \in G} V_g$. Morphisms are linear maps preserving the grading. The tensor product is defined by
...
%% FIXME: prove same rep for equivalent cats? move above to later section
...
The following definitions and theorem are due to Kirillov Jr.
...
\end{aligned}
\end{equation}
up to associators.  Note that, due to the order of $X$ and $\lstar X$ in the tensor product, we use the right evaluation map $\ev_{\lstar X} \colon X \otimes  \lstar X \to \one$ for the left dual $\lstar X$, instead of the right evaluation map $\ev_X : X^* \otimes X \to 1$ for $X$.  
...
%% FIXME: Prove that the Turaev-Viro/ RT construction gives the same thing for equivalent TQFTs
...
\begin{figure}
 ...
 \caption{Simple closed curves for the Dehn twists in the Lickorish generating set for the mapping class group of a genus $3$ closed surface (Image source: \texttt{https://en.wikipedia.org/wiki/Dehn\_twist})}
\label{fig:lickorish}
\end{figure}
...
When $\Si$ has boundary, we denote by $\MCG(\Si)$ the group of isotopy classes of homeomorphisms fixing the boundary of $\Si$ setwise. Given any labelling of the boundary  by objects in the Drinfeld center, $l : \pi_0(\partial M) \to \Obj(Z(\vgo))$, we get a mapping class group representation. The representation space is $\Hs(\Si, \VV)$ where the boundary condition $\VV = F \circ l$, where $F$ is the forgetful functor $F: Z(\vgo) \to \vgo$.  The same local relations are valid in this representation space \cite{kirillovStringNets}.
...
\bibitem{funar} L.\ Funar, \emph{On the TQFT representations of the mapping class groups}, Pacific J.\ Math\., \textbf{188} (1999), 251--274.
...
\bibitem{We} C.\ Weibel, {\em An introduction to Homological algebra}, Cambridge Studies in Advanced Mathematics \textbf{38} (1995).

\bibitem{gw} G.\ Wright, \emph{The Reshetikhin-Turaev representation of the mapping class groups}, J.\ Knot Theory Ramifications \textbf{3} (1994),
547--574.
