...
\cite{kirillovStringNets}.  An unstrictified version is recorded here for convenience.  For any spherical fusion category $\mathcal A$ and surface $\Si$, Kirillov gives the following presentation of the Levin-Wen model as a vector space of
...
\ph\otimes\psi & \mapsto \ph\cc{X}\psi= (\id_{V_1 \otimes \cdots \otimes V_{n}} \otimes \ev_{\lstar X}  \otimes \id_{W_1 \otimes \cdots \otimes W_{m}} )\circ (\ph\otimes\psi),
\end{aligned}
\end{equation}
up to associators.  % Note that, due to the order of $X$ and $\lstar X$ in the tensor product, we use the right evaluation map $\ev_{\lstar X} = \lstar\coev_X \colon X \otimes  \lstar X \to \one$ for the left dual of $X$, instead of the right evaluation map $\ev_X : X^* \otimes X \to 1$ for $X$.  
...
following data:
  ...
  \begin{itemize}
    \item Choice of a pair $V(\ee)\in \Obj \A$ for every oriented edge $\ee\in E^{or}(\Ga)$ so that $V(\ov{\ee}) = V(\ee)^*$.
