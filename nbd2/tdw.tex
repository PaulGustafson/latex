...
\cite{kirillovStringNets}.  An unstrict version is recorded here for convenience.  For any spherical fusion category $\mathcal A$ and surface $\Si$, Kirillov gives the following presentation of the Levin-Wen model as a vector space of
...
\ph\otimes\psi & \mapsto \ph\cc{X}\psi= (\id_{V_1 \otimes \cdots \otimes V_{n}} \otimes \ev_{X}  \otimes \id_{W_1 \otimes \cdots \otimes W_{m}} )\circ (\ph\otimes\psi),
\end{aligned}
\end{equation}
up to associators and unitors.  % Note that, due to the order of $X$ and $\lstar X$ in the tensor product, we use the right evaluation map $\ev_{\lstar X} = \lstar\coev_X \colon X \otimes  \lstar X \to \one$ for the left dual of $X$, instead of the right evaluation map $\ev_X : X^* \otimes X \to 1$ for $X$.  
...
following data:
  ...
  \begin{itemize}
    ...
    \item Choice of a vector $\ph(v)\in \<V(\ee_1)^{\epsilon_1},\dots,V(\ee_n)^{\epsilon_n}\>$ 
      ...
      order. The object $V(\ee_i)^{\epsilon_i} = V(\ee_i)$ if $\ee_i$ is outgoing and $V(\ee_i)^*$ if $\ee_i$ is incoming. % (see \firef{f:coloring}). 
\end{itemize}

  An {\em isomorphism} $f$ of two colorings $\{V(\ee), \ph(v)\}$, $\{V'(\ee), \ph'(v)\}$ is a collection of isomorphisms $f_\ee \colon V(\ee)\cong V'(\ee)$  and $f_{\overline \ee}: V(\ee)^* \cong V'(\ee)^*$ for which  $\ph'(v)=f\circ\ph(v)$.
%% FIXME: how to make f_\overline \ee???
