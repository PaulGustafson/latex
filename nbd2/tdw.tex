...
following data:
  ...
  \begin{itemize}
    ...
    \item Choice of a vector $\ph(v)\in \<V(\ee_1)^{\epsilon_1},\dots,V(\ee_n)^{\epsilon_n}\>$ 
      ...
      order. The object $V(\ee_i)^{\epsilon_i} = V(\ee_i)$ if $\ee_i$ is outgoing from $v$ and $V(\ee_i)^*$ if $\ee_i$ is incoming. % (see \firef{f:coloring}). 
\end{itemize}
...
{$\coev_{\lstar V}$};
\draw[->] (ph)-- +(180:1cm) node[pos=1.0,above,scale=0.8] {$\lstar V$};
...
%%%%%%%%
\begin{tikzpicture}
\draw[->] (0,0)-- (2,0) node[pos=0.5,above,scale=0.8] {$V$};
\end{tikzpicture}
$$
%%%%%%%%%%%%%%%%%%%%%%%%%%
\caption{Local relations for colored graphs with associators and unitors suppressed.        }\label{f:local_rels1}
%% FIXME: adding a coev does something bad.  In the strictification, it's (g, 1).   Here it's (g^*, 0).
