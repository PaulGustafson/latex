...
$$z(\phi)= (j_{\lstar \lstar V}^{-1} \otimes \id_{V_1 \otimes \cdots \otimes V_{n-1}} \otimes \ev_{\lstar V_n} ) \circ (\id_{\lstar \lstar V_n} \otimes \phi \otimes \id_{\lstar V_n} ) \circ  \coev_{\lstar \lstar V_n}  $$ 
...
\end{aligned}
\end{equation}
where $\ev_{\lstar X} \colon X \otimes  \lstar X \to \one$ is the evaluation morphism.
...
\end{prop}
\begin{proof}
The proof of (1) follows from the definition of the local moves. For (2), we'll consider each local move separately.  In each case, we need to show that $\Delta$ is equivalent to $\alpha \Delta'$ in $H$.  This boils down to using the linearity of the evaluation map of Theorem \ref{t:RT} with respect to each vertex label.
