...
\end{equation}
where, up to associators, if $\phi \in \Hom_\A(\one, V_1 \otimes \cdots \otimes V_n)$ then
$z(\phi)= \coev_{V_n} \circ () $ %FIXME
...
\begin{aligned}
 \<V_1,\dots,V_n, X\> \otimes \<\stX, W_1,\dots,
W_m\>&\to\<V_1,\dots,V_n, W_1,\dots, W_m\> \\
\ph\otimes\psi\mapsto \ph\cc{X}\psi= \ev_{\stX} \circ (\ph\otimes\psi)
\end{aligned}
\end{equation}
where $\ev_{\stX} \colon X \otimes  \stX \to \one$ is the evaluation morphism.
%% FIXME: strict pivotal??
...
Let $\Gamma$ be a simple colored graph embedded in a surface $\Sigma$.  Let $\Delta$ be the colored graph given by applying any local move in Figure \ref{f:local_rels1} to $\Gamma$.  Then
\begin{enumerate}
\item  each edge of $\Delta$ is labeled by $\delta_g$ for some $g \in G$, and
\item  there exists $\alpha \in \Img(\omega) \cup \{1\}$ such that 
$$\Delta - \alpha \Delta' \in N(\Sigma, \VV),$$
where $\Delta'$ is the simple colored graph given by replacing each vertex label in $\Delta$ with the canonical isomorphism in condition (2) of Definition \ref{def:vect}.
\end{enumerate}
\end{prop}
\begin{proof}
The proof of (1) follows from the definition of the local moves. For (2), we'll consider each local move separately.  In each case, we need to show that $\Delta$ is equivalent to $\alpha \Delta'$ in $H$.  This boils down to using the linearity of the evaluation map of Theorem \ref{\t:rt} with respect to the vertex coloring.

For the first (edge contraction) local move in Figure \ref{f:local_rels1}, %FIXME
sing the same notation as in the figure, we repeatedly apply associators and the cyclic $z$-morphism of Equation \ref{e:cyclic} so that the order of objects in the codomains of $\phi$ and $\psi$ correspond to the orders on the left hand side of Equation \ref{e:composition} and that $X$ and $X^*$ are isolated (not contained in any parentheses).  After applying the $\ev_X$ morphism, we reassociate until the new label $\ph\cc{X}\psi$ has the appropriate parenthesization.  Since every edge is labeled by a simple object, each associator morphism is  of the form $\alpha_{g,h,k}$ for some $g,h,k \in G$.  For the same reason, every $z$-morphism is a composition of tensor products of morphism of the form $\id_{g}$, $\ev_g$, $\coev_g$ , $\alpha_{g,h,k}$, and $j_g$.  By the definition of $\Vect_G^\omega$ (in particular, the definitions of the structural morphisms on the simple objects), it follows that the result of the local moves is of the desired form.
...
When $\Si$ has boundary, we denote by $\MCG(\Si)$ the group of isotopy classes of homeomorphisms fixing the boundary of $\Si$ setwise. There are mapping class group representations for every labelling of the boundary  $l : \pi_0(\partial M) \to \Obj(Z(\vgo))$.  Given such a labelling, the representation space is $\Hs(\Si, \VV)$ where the boundary condition $\VV = F \circ l$, where $F$ is the forgetful functor $F: Z(\vgo) \to \vgo$.  The same local relations are valid in this representation space \cite{kirillovStringNets}.
...
%% \bibitem{We} Weibel, C., An introduction to Homological algebra,
