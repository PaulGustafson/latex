...
Let $\Gamma$ be a simple colored graph embedded in a surface $\Sigma$.  Then any local move in Figure \ref{f:local_rels1} applied to $\Gamma$ results in a colored graph equivalent to $\alpha \Delta$ for some $\alpha \in \Img(\omega)$ and simple colored graph $\Delta$.
\end{prop}
\begin{proof}
For the first (edge contraction) local move, there are two steps.  First, we repeatedly apply associators and the cyclic $z$-morphism of Equation \ref{e:cyclic} so that the $\phi $

This follows from the fact that all the structural morphisms of $\Vect_G^\omega$ evaluated on simple objects are of the form $\alpha \id_X$ where $\alpha \in \Img(\omega)$.  
